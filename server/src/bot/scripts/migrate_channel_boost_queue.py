...
CREATE TABLE IF NOT EXISTS channel_boost_queues (
    id SERIAL PRIMARY KEY,
    ad_id INTEGER NOT NULL UNIQUE REFERENCES ads(id) ON DELETE CASCADE,
    enqueued_at TIMESTAMP NOT NULL DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS ix_channel_boost_queue_enqueued_at ON channel_boost_queues (enqueued_at);
"""


async def run_migration():
    print("Миграция: channel_boost_queues + last_channel_boost_post_at...")
